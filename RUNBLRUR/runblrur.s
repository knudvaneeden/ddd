...
FORWARD INTEGER PROC FNLineRunProgramB()
...
FORWARD PROC Main()
FORWARD PROC PROCBlockChangeLineToColumnType()
FORWARD PROC PROCBlockRunUrlAll( STRING s1, INTEGER i1 )
...
PROC Main()
 // PROCBlockRunUrlAll( "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", 100 )
 PROCBlockRunUrlAll( "c:\program files\google\chrome\application\chrome.exe", 200 )
END
...
// library: block: run: url: all <description></description> <version control></version control> <version>1.0.0.0.41</version> <version control></version control> (filenamemacro=runblrur.s) [<Program>] [<Research>] [kn, ri, mo, 07-11-2016 20:59:38]
PROC PROCBlockRunUrlAll( STRING browserExecutableS, INTEGER urlMaxI )
 // e.g. PROC Main()
 // e.g.  // PROCBlockRunUrlAll( "C:\Program Files (x86)\Google\Chrome\Application\chrome.exe", 100 )
 // e.g.  PROCBlockRunUrlAll( "c:\program files\google\chrome\application\chrome.exe", 200 )
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 ...
 //
 INTEGER blockRowBeginI = 0
 INTEGER blockRowEndI = 0
 //
 INTEGER lineBlockTotalI = 0
 //
 STRING s[255] = ""
 //
 INTEGER B = FALSE
 ...
 IF ( IsBlockInCurrFile() == _LINE_ )
  PROCBlockChangeLineToColumnType()
 ENDIF
 //
 blockColumnBeginI = Query( blockBegCol )
 blockColumnEndI = Query( blockEndCol )
 //
 blockRowBeginI = Query( blockBegLine )
 blockRowEndI = Query( blockEndLine )
 //
 lineBlockTotalI = blockRowEndI - blockRowBeginI + 1
 //
 IF ( ( lineBlockTotalI) > ( urlMaxI ) )
  //
  IF ( FNMathGetNumberInputYesNoCancelPositionDefaultI( Format( lineBlockTotalI, " ", "lines selected in the block. But loading more than", " ", urlMaxI, " ", "URL TAB your (Chrome) browser might hang or slow down your (Chrome) browser. Please select less lines in the block. Stop?" ) ) == 1 )
   //
   PopBlock()
   //
   RETURN()
 ...
 WHILE ( ( IsCursorInBlock() ) AND ( downB ) )
  ...
  //
  IF ( LFind( "http", "cgi" ) ) OR ( LFind( "https", "cgi" ) ) OR ( LFind( "file:///", "cgi" ) )
   //
   // s = GetText( blockColumnBeginI, blockColumnEndI - blockColumnBeginI + 1 ) // old [kn, ri, su, 20-09-2020 16:34:37]
   // s = GetText( 1, 255 ) // new [kn, ri, su, 20-09-2020 16:34:34] // old [kn, ri, tu, 18-10-2022 15:04:09]
   // s = FNStringGetCarS( s ) // take off from front of the line until the first space (because URLs should not contain any space anyhow) // old [kn, ri, tu, 18-10-2022 15:05:13]
   //
   PushPosition()
   PushBlock()
   PROCMacroRunPurge( "marktexs" )
   s = GetText( Query( BlockBegCol ), Query( BlockEndCol ) - Query( BlockBegCol ) + 1 )
   PopBlock()
   PopPosition()
   //
   // PROCProgramRunInternetBrowserGoogleChromeAlias( GetText( blockColumnBeginI, blockColumnEndI - blockColumnBeginI + 1 ) )
   // StartPgm( "chrome", GetText( blockColumnBeginI, blockColumnEndI - blockColumnBeginI + 1 ) ) // will work, but will not open the TABs in the browser exactly in order from top to bottom of URLs in the block
   // Dos( Format( "cmd.exe", " ", "/c", " ", QuotePath( browserExecutableS ), " ", s ), _DONT_PROMPT_ )
   Dos( Format( "cmd.exe", " ", "/c", " ", QuotePath( browserExecutableS ), " ", s ), _DONT_PROMPT_ | _START_HIDDEN_ )
   //
  ELSE
   //
   B = FNLineRunProgramB()
...
// library: block: change: line: to: column: type <description></description> <version>1.0.0.0.6</version> <version control></version control> (filenamemacro=chanblcy.s) [<Program>] [<Research>] [kn, ri, we, 11-01-2012 22:12:28]
PROC PROCBlockChangeLineToColumnType()
 // e.g. PROC Main()
 // e.g.  PROCBlockChangeLineToColumnType()
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 INTEGER bufferI = 0
 //
 INTEGER columnMinI = 0
 //
 INTEGER columnMaxI = 0
 //
 INTEGER lineMinI = 0
 //
 INTEGER lineMaxI = 0
 ...
 PushPosition()
 //
 IF ( ( IsBlockInCurrFile() == _LINE_ | _INCLUSIVE_ | _NON_INCLUSIVE_ ) )
  //
  lineMinI = Query( blockBegLine )
  //
  lineMaxI = Query( blockEndLine )
  //
  Copy()
  //
  PushPosition()
  ...
  Paste()
  //
  columnMaxI = LongestLineInBuffer()
  //
  GotoBufferId( bufferI )
  //
  AbandonFile()
  ...
  PopPosition()
  //
  MarkColumn( lineMinI, columnMinI, lineMaxI, columnMaxI )
  //
 ENDIF
 //
 PopPosition()
...
// library: line: run: program <description></description> <version control></version control> <version>1.0.0.0.14</version> <version control></version control> (filenamemacro=runlirpr.s) [<Program>] [<Research>] [kn, ri, mo, 15-06-2020 11:24:08]
INTEGER PROC FNLineRunProgramB()
 // e.g. PROC Main()
 // e.g.  Message( FNLineRunProgramB() ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 ...
 PushBlock()
 //
 IF LFind( '^[ ]*{[a-zA-Z]:.*{[ ]|$}}', "cgx" ) OR LFind( '^[ ]*{"[a-zA-Z]:.*{"|$}}', "cglx" )
  s = GetFoundText( 1 )
  B = TRUE
  //
  // Make sure that it is always run using Acrobat (instead of e.g. Adobe Reader if that is installed).
  // You might have to removed these below lines on your system if you do not want this enacted.
  //
  // IF EquiStr( ".pdf", SplitPath( s, _EXT_ ) )
  //  StartPgm( "acrobat", QuotePath( s ) )
  // ELSE
   StartPgm( s )
  // ENDIF
 ENDIF
