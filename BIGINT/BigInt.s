/*
  ...
  This macro implements a simple calculator for signed integers of up to 255
  characters.

  It supports addition, subtraction, multiplication, division and comparison.

  You can paste the Windows clipboard into its prompts, and results are copied
  to the Windows clipboard.

  To facilitate a broad range of input, you may enter integers with multiple
  leading and trailing signs and spaces, and with at most one group separator
  between any two digits to account for many country customs and other styles.
  Valid group separators are; comma, period, space, apostrophe and underscore.
  Decimal separators are not recognized as such: They are ignored if they
  resemble valid group separators, and cause an error otherwise.

  For example:
    "+ -- 1.234 +-+" and " -0.0'1,2 3_4 " are both interpreted as "-1234".
    "1-2", "12.", "_12", "1  2" and "1,,2" are all invalid input integers.

  An "ERROR" result will be reported for an invalid input number, a division by
  zero, or a result that would contain more than 255 characters.


  INSTALLATION

  Just copy this ".s" file to TSE's "mac" folder, and compile it there,
  for instance by opening it in TSE and applying TSE's Macro Compile menu.

  You can ignore the BigInt.inc file: It is not necessary.
  It contains information for macro programmers.

  You can start the calculator by executing "BigInt" as a macro, for instance
  by typing that name in in TSE's Macro Execute menu.


  MACRO PROGRAMMER INFO

  To any macro programmers out there.
  I maintain the math functions in an independent separate file "BigInt.inc"
  for maintenence reasons that are explained therein, and I have copied the
  contents of that file as a whole into this macro.
  ...
  v1.3 - 6 Jan 2020
    ...
    making each of them version v1.3, and then copied the content of the final
    version of the math functions file back into the calculator.
    Renamed variable names to significant names.
    Improved the user interface and the documentation.
*/
...
// End of #Include ['BigInt.inc']





proc display_result(string operand1, string operation, string operand2, string p_result)
  integer maxsize               = 0
  string  result [MAXSTRINGLEN] = iif(p_result == '', 'ERROR', p_result)
  maxsize = Max(Max(Max(Max(Length(operand1),
                            Length(operand2)),
                        Length(result)),
                    Length('   ' + operation)),
                Length('   equals'))
  CopyToWinClip(result)
  Message('The result has been stored in the Windows clipboard.')
  // The following weird buffer format is to get consistent results
  // from MsgBoxBuff() for both very large and very small integers.
  // This as a work-around for MsgBoxBuff(), MsgBoxEx(), MsgBox() and Warn()
  // not honouring Chr(13) after a string longer than the screen width,
  // which I reported as a bug to Semware on 5 Jan 2020.
  EmptyBuffer()
  AddLine(Format(operand1:maxsize))
  AddLine(' ')
  AddLine(Format('   ' + operation))
  AddLine(' ')
  AddLine(Format(operand2:maxsize))
  AddLine(' ')
  AddLine(Format('   equals'))
  AddLine(' ')
  AddLine(Format(result:maxsize))
  MsgBoxBuff('Calculation')
end display_result

proc Main()
  integer action                  = 0
  integer comparison              = 0
  string  operand1 [MAXSTRINGLEN] = ''
  string  operand2 [MAXSTRINGLEN] = ''
  ...
  repeat
    ...
    action = MsgBoxEx('Big Integers', 'Select action',
                      '[&Add];[&Subtract];[&Multiply];[&Divide];[&Compare]')
    if action == 1
      if  Ask('Enter the first big integer to add:' , operand1)
      and Ask('Enter the second big integer to add:', operand2)
        result = add_bigints(operand1, operand2)
        display_result(operand1, 'plus', operand2, result)
      endif
    elseif action == 2
      if  Ask('Enter the big integer to subtract from:', operand1)
      and Ask('Enter the big integer to subtract:'     , operand2)
        result = subtract_bigint_minus_bigint(operand1, operand2)
        display_result(operand1, 'minus', operand2, result)
      endif
    elseif action == 3
      if  Ask('Enter the first big integer to multiply:' , operand1)
      and Ask('Enter the second big integer to multiply:', operand2)
        result = multiply_bigints(operand1, operand2)
        display_result(operand1, 'multiplied by', operand2, result)
      endif
    elseif action == 4
      if  Ask('Enter the big integer to divide:', operand1)
      and Ask('Enter the big integer to divide by:'     , operand2)
        result = divide_bigint_by_bigint(operand1, operand2)
        display_result(operand1, 'divided by', operand2, result)
      endif
    elseif action == 5
      if  Ask('Enter the first big integer to compare:' , operand1)
      and Ask('Enter the second big integer to compare:', operand2)
        comparison = compare_bigints(operand1, operand2)
        case comparison
          when -1
            result = 'FIRST SMALLER THAN SECOND'
          when  0
            result = 'FIRST EQUALS SECOND'
          when  1
            result = 'FIRST GREATER THAN SECOND'
          otherwise
            result = 'ERROR'
        endcase
        display_result(operand1, 'compared to', operand2, result)
      endif
    endif
  until action == 0
  Message('')
