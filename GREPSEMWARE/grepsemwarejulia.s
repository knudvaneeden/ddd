FORWARD INTEGER PROC FNProgramRunSearchGrepTseJuliaB()
FORWARD PROC Main()
...
PROC Main()
 Message( FNProgramRunSearchGrepTseJuliaB() ) // gives e.g. TRUE
END
...
// library: program: run: search: grep: tse: julia <description></description> <version control></version control> <version>1.0.0.0.1</version> <version control></version control> (filenamemacro=runprtju.s) [<Program>] [<Research>] [kn, ri, su, 04-12-2022 22:38:14]
INTEGER PROC FNProgramRunSearchGrepTseJuliaB()
 // e.g. PROC Main()
 // e.g.  Message( FNProgramRunSearchGrepTseJuliaB() ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 // ===
 //
 // Use case = Convert Julia grep output to TSE grep output.
 //
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNProgramRunSearchGrepTseJuliaB )
 // e.g. HELPDEF HELPDEFFNProgramRunSearchGrepTseJuliaB
 // e.g.  title = "FNProgramRunSearchGrepTseJuliaB() help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 ...
 PushBlock()
 //
 // delete any lines not starting with a filename (e.g. C:, D:, E:, ...). Goal is to remove Julia warnings, ...
 //
 GotoBlockBegin()
