FORWARD INTEGER PROC FNProgramRunSearchGrepTseMingwB()
FORWARD PROC Main()
...
PROC Main()
 Message( FNProgramRunSearchGrepTseMingwB() ) // gives e.g. TRUE
END
...
// library: program: run: search: grep: tse: mingw <description></description> <version control></version control> <version>1.0.0.0.1</version> <version control></version control> (filenamemacro=grepsemwaremingw.s) [<Program>] [<Research>] [kn, ri, su, 04-12-2022 22:36:51]
INTEGER PROC FNProgramRunSearchGrepTseMingwB()
 // e.g. PROC Main()
 // e.g.  Message( FNProgramRunSearchGrepTseMingwB() ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 // ===
 //
 // Use case = Convert Mingw grep output to TSE grep output.
 //
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNProgramRunSearchGrepTseMingwB )
 // e.g. HELPDEF HELPDEFFNProgramRunSearchGrepTseMingwB
 // e.g.  title = "FNProgramRunSearchGrepTseMingwB() help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 ...
 PushBlock()
 //
 // delete any lines not starting with a filename (e.g. C:, D:, E:, ...). Goal is to remove Mingw warnings, ...
 //
 GotoBlockBegin()
