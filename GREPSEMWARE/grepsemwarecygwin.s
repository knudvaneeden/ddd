FORWARD INTEGER PROC FNProgramRunSearchGrepTseCygwinB()
FORWARD PROC Main()
...
PROC Main()
 Message( FNProgramRunSearchGrepTseCygwinB() ) // gives e.g. TRUE
END
...
// library: program: run: search: grep: tse: cygwin <description></description> <version control></version control> <version>1.0.0.0.3</version> <version control></version control> (filenamemacro=grepsemwarecygwin.s) [<Program>] [<Research>] [kn, ri, su, 04-12-2022 21:49:01]
INTEGER PROC FNProgramRunSearchGrepTseCygwinB()
 // e.g. PROC Main()
 // e.g.  Message( FNProgramRunSearchGrepTseCygwinB() ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 // ===
 //
 // Use case = Convert Cygwin grep output to TSE grep output.
 //
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNProgramRunSearchGrepTseCygwinB )
 // e.g. HELPDEF HELPDEFFNProgramRunSearchGrepTseCygwinB
 // e.g.  title = "FNProgramRunSearchGrepTseCygwinB() help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 ...
 PushBlock()
 //
 // delete any lines not starting with a filename (e.g. C:, D:, E:, ...). Goal is to remove Cygwin warnings, ...
 //
 GotoBlockBegin()
