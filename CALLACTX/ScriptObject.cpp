...
void CScriptObject::CommonConstruct()
{
	...
	// _tcscpy(m_szLanguage, LANGUAGE_DEFAULT);  // temporarily commented out for clean compile
	m_pScript->PutAllowUI( VARIANT_FALSE );
	//m_pScript->PutLanguage( _bstr_t(m_szLanguage ) ); // temporarily commented out for clean compile
	// m_pScript->PutLanguage( LANGUAGE_DEFAULT );
}
...
{
	// _tcscpy(m_szLanguage, szLanguage); // temporarily commented out for clean compile

	if (m_pScript != NULL)
	{
		// m_pScript->PutLanguage( _bstr_t( szLanguage ) ); // temporarily commented out for clean compile
		m_pScript->PutLanguage( "VBScript" );
		m_pScript->Reset();
	...
	if (m_pScript != NULL)
	{
		//try{
			...
			if (pError != NULL)
			{
				...
				// _tcsncpy(m_szError, (LPCTSTR) desc, count);  // temporarily commented out for clean compile
				// m_szError[count] = 0; // temporarily commented out for clean compile
			   pError->Clear();
			}
		//}
		//catch(_com_error& e)
		//{
			//  TRACE( (LPSTR)e.Description() ); // temporarily commented out for clean compile
			// TRACE( (LPSTR)"\n" ); // temporarily commented out for clean compile
		//}
	}
	return 0;
	// return m_szError; // temporarily commented out for clean compile
}
...
bool CScriptObject::LoadScriptResource(LPCTSTR lpName, LPCTSTR lpType, HINSTANCE hInstance )
{
	try{
		if (m_pScript)
		{
			...
			// ASSERT( res != NULL); // temporarily commented out for clean compile
			BYTE* pbytes = (BYTE*) LockResource(LoadResource(hInstance, res ));
			// ASSERT( pbytes != NULL); // temporarily commented out for clean compile
...
{
	// required argument
	// ASSERT( saParameters != NULL); // temporarily commented out for clean compile
	// ASSERT( varRet  != NULL); //  temporarily commented out for clean compile
